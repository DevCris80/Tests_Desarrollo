...
class TestGames:
    ...
    def test_piedra_papel_tijera(self):
        ...
        assert (
            self.games.piedra_papel_tijera("piedra", "tijera") == "jugador1"
        )  # Piedra vence tijera
        assert (
            self.games.piedra_papel_tijera("papel", "piedra") == "jugador1"
        )  # Papel vence piedra
        assert (
            self.games.piedra_papel_tijera("tijera", "papel") == "jugador1"
        )  # Tijera vence papel

        # Test victorias de jugador2
        assert (
            self.games.piedra_papel_tijera("tijera", "piedra") == "jugador2"
        )  # Piedra vence tijera
        assert (
            self.games.piedra_papel_tijera("piedra", "papel") == "jugador2"
        )  # Papel vence piedra
        assert (
            self.games.piedra_papel_tijera("papel", "tijera") == "jugador2"
        )  # Tijera vence papel
    ...
    def test_generar_combinacion_mastermind(self):
        ...
        assert (
            len(set(combinaciones)) > 1
        )  # Debe generar al menos 2 combinaciones distintas
    ...
    def test_validar_movimiento_torre_ajedrez(self):
        ...
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 0, 7, tablero_vacio)
            == True
        )  # Fila completa
        assert (
            self.games.validar_movimiento_torre_ajedrez(4, 2, 4, 6, tablero_vacio)
            == True
        )  # Movimiento parcial
        assert (
            self.games.validar_movimiento_torre_ajedrez(7, 7, 7, 0, tablero_vacio)
            == True
        )  # Reversa

        # Test movimientos verticales válidos
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 7, 0, tablero_vacio)
            == True
        )  # Columna completa
        assert (
            self.games.validar_movimiento_torre_ajedrez(2, 4, 6, 4, tablero_vacio)
            == True
        )  # Movimiento parcial
        assert (
            self.games.validar_movimiento_torre_ajedrez(7, 3, 1, 3, tablero_vacio)
            == True
        )  # Hacia arriba

        # Test movimiento diagonal (inválido para torre)
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 1, 1, tablero_vacio)
            == False
        )
        assert (
            self.games.validar_movimiento_torre_ajedrez(3, 3, 5, 5, tablero_vacio)
            == False
        )

        # Test movimiento a la misma posición (inválido)
        assert (
            self.games.validar_movimiento_torre_ajedrez(4, 4, 4, 4, tablero_vacio)
            == False
        )
        ...
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 0, 7, tablero_con_piezas)
            == False
        )
        # Movimiento que no pasa por el obstáculo
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 0, 2, tablero_con_piezas)
            == True
        )
        # Movimiento después del obstáculo
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 4, 0, 7, tablero_con_piezas)
            == True
        )
        ...
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 7, 0, tablero_con_piezas2)
            == False
        )
        # Movimiento que no pasa por el obstáculo
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 2, 0, tablero_con_piezas2)
            == True
        )

        # Test movimientos fuera del tablero (deben ser inválidos)
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 0, 8, tablero_vacio)
            == False
        )
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, 0, 8, 0, tablero_vacio)
            == False
        )
        assert (
            self.games.validar_movimiento_torre_ajedrez(-1, 0, 0, 0, tablero_vacio)
            == False
        )
        assert (
            self.games.validar_movimiento_torre_ajedrez(0, -1, 0, 0, tablero_vacio)
            == False
        )
